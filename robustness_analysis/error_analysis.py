...
os.environ["TFRECORDS_PATH"] =  "/data/raw/siq_audio_transformation"
...
class MerlotReserveTVQA(MerlotReserve):
    ...
    def __call__(self, batch):
        ...
        #imgs_enc = jnp.full_like(imgs_enc, 0)
        #########################
        ...
        #batch['audio_clips']= jnp.full_like(batch['audio_clips'],0)
...
def val_epoch(state: train_state.TrainState):
    ...
    text_preds.to_csv('at_text_preds.csv')
    audio_preds.to_csv('at_audio_preds.csv')
    joint_preds.to_csv('at_joint_preds.csv')
