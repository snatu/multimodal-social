...
class MerlotReserveTVQA(MerlotReserve):
    ...
    def __call__(self, batch):
        ...
        if args.disable_audio:
             x = audio_inputs['x']
             coords = audio_inputs['rotary_coords']
             attnmask = audio_inputs['attention_mask']
             ...
             logits_from_audio = jnp.squeeze(self.proj(pooled_h), -1)
             logits_from_audio = logits_from_audio.reshape(batch_size, num_ans_per)
             logits_from_text = jnp.ones_like(logits_from_audio)
