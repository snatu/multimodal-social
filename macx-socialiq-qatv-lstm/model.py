...
class DecoderRNN(nn.Module):
  ...
  def forward(self, features, captions):
    ...
    #features dimension 1x32xembed_hidden*2
    ...
    #eatures dimension 1x32xembed_hidden*2
    
    #print(features.unsqueeze(0).shape,embeddings.shape)
    #embeddings = torch.cat((features.unsqueeze(0), embeddings), dim=0)
    #embeddings = torch.cat((features, ans), dim=1)
    embeddings = features
    #print("embedding shape",embeddings.shape)
    #print("embedding after concat shape",embeddings.shape, "features shape", features.shape, "ans", ans.shape)
    hiddens, _ = self.lstm(embeddings)
    #print("lstm out of decoder should be of shape embed_hidden")
    
    outputs = self.linear(hiddens)
    #print("final output shape will be vocab shape")
    return outputs
...
class MACNetwork(nn.Module):
    def __init__(self, kb_dim, dim, embed_hidden=768,
        ...
        self.lstm_decoder = DecoderRNN(embed_hidden*2+1024,embed_hidden,1024)

    def forward(self, kb, question, answers, corr_ans_txt):
        b_size = question.shape[0]
        
        lstm_out_q, (h_q, _) = self.lstm(question)
        h_q = h_q.permute(1, 0, 2).contiguous().view(b_size, -1)
        #print("raw answer or a1",answers[0].shape)
        
        h_as = []
        for answer in answers:
            lstm_out_a, (h_a, _) = self.lstm(answer)
            #print("lstm out of a1", lstm_out_a.shape)
        ...
        intermediate1 = torch.cat([h_q, memory, h_as[0]],1)
        intermediate2 = torch.cat([h_q, memory, h_as[1]],1)
        
        #torch.cat([h_q, memory, *h_as], 1) #self.classifier(torch.cat([h_q, memory, *h_as], 1))#torch.cat([h_q, memory],1)
        #print('hidden rep of answer', h_as[0].shape)
        lstm_forward1 = self.lstm_decoder(intermediate1,corr_ans_txt)
        lstm_forward2 = self.lstm_decoder(intermediate2,corr_ans_txt)
        
        #print("lstm_forward shape", lstm_forward.shape)
        
        #2*32*256
        #print(lstm_forward)
    
        
        return self.classifier(torch.cat([h_q, memory, *h_as], 1)), lstm_forward1, lstm_forward2, h_as
...
class MACNetwork_2RUs(nn.Module):
    ...
    def forward(self, kb1, kb2, question, answers, corr_ans_txt):
        b_size = question.shape[0]
...
class MACNetwork_3RUs(nn.Module):
    ...
    def forward(self, kb1, kb2, kb3, question, answers,corr_ans_txt):
        b_size = question.shape[0]
