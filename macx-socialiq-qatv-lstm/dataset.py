...
class SocialIQ(Dataset):
    ...
    def __init__(self, root, phase, mods, a4=False):
        ...
        self.h_qa = None
        if not a4:
            self.h_qa = h5py.File(self.root + 'SOCIAL-IQ_QA_BERT_LASTLAYER_BINARY_CHOICE.csd', 'r')
        else:
            self.h_qa = h5py.File(self.root + 'SOCIAL-IQ_QA_BERT_MULTIPLE_CHOICE.csd', 'r')

        self.h_v = None
        if 'v' in mods:
            self.h_v = h5py.File(self.root + 'SOCIAL_IQ_DENSENET161_1FPS.csd', 'r')

        self.h_t = None
        if 't' in mods:
            self.h_t = h5py.File(self.root + 'SOCIAL_IQ_TRANSCRIPT_RAW_CHUNKS_BERT.csd', 'r')

        self.h_ac = None
        if 'ac' in mods:
            self.h_ac = h5py.File(self.root + 'SOCIAL_IQ_COVAREP.csd', 'r')
        ...
        for sid in tqdm(fold):
            ...
            for i_q, question in enumerate(qa_embs):
                for i_c, combination in enumerate(question):
                    ...
                    #print(combination[1:].shape,len(enum))
                    random.shuffle(enum)
