...
TRAIN_SIZE = 21 * config['data']['num_train_files'] # config['data']['num_train_files'] * RECORD_PER_FILE if PERCENT_DATASET < 1.0 else TOTAL_SIZE
...
if (jax.process_index() == 0) and not args.no_wandb:
    import wandb
    wandb.init(config=config, project=args.wandb_name, entity='sherylm', name=args.wandb_run_name, notes=f'Loaded from {cfg_name}', tags=tags)
else:
    wandb = None
    ...
    if jax.process_index() == 0:
        ...
        if step_for_logging >= 0:
            ...
            for k, v in train_metrics[step_for_logging].items():
                if k == "audio2vision":
                    audio2vision_loss = float(v)
                if k == "text2vision":
                    text2vision_loss = float(v)
            if not args.no_wandb:
                # wandb.log(train_metrics[step_for_logging], step=step_for_logging, commit=(n + 1) % log_every == 0)
                wandb.log({"audio2vision_loss": audio2vision_loss}, step=step_for_logging, commit=(n + 1) % log_every == 0)
                wandb.log({"text2vision_loss": text2vision_loss}, step=step_for_logging, commit=(n + 1) % log_every == 0)
