...
from pretrain.data_utils import uniform_random_select
from pretrain.dataloader import _one_hot
...
def get_mask_subset_with_prob(patched_input, prob):
    ...
    return new_mask
@jit
def f_p(vision_input):
    return vision_input.reshape((np.prod(vision_input.shape),))
...
class TransformerEncoder(nn.Module):
    ...
    def __call__(self, x, *, rotary_coords=None, attention_mask=None, is_valid=None):
        ...
        :return:
        """
        *batch_dims, seq_len, hsz = x.shape # (7, 540, 768)
        assert hsz == self.hidden_size
        ...
        if self.add_cls_token:
            ...
            cls_token = self.param('cls', nn.initializers.normal(stddev=0.02), (self.hidden_size,)) # (768,)
            ...
            cls_token = jnp.tile(cls_token, list(batch_dims) + [1, 1]) # (7, 1, 768)
            x = jnp.concatenate([cls_token.astype(x.dtype), x], -2) # (7, 541, 768)
            if is_valid is not None:
                is_valid = jnp.concatenate([jnp.ones(list(batch_dims) + [1], dtype=jnp.bool_), is_valid], -1)

            # rotary_coords.shape = (540, 2)
            if rotary_coords is not None:
                # CLS token is always 0's
                rotary_coords = jnp.concatenate([jnp.zeros_like(rotary_coords[..., :1, :]), rotary_coords], -2) # (541, 2)
        ...
        if rotary_coords is not None:
            *rotary_batch_dims, seq_len_, rotary_axes = rotary_coords.shape
            ...
            sinusoids = construct_rotary_sinusoids(rotary_coords, rotary_hsize=self.rotary_hsize) # (2, 541, 32)
        else:
            sinusoids = None
        ...
        x = nn.LayerNorm(epsilon=1e-5, dtype=self.dtype, name='pre_ln')(x) # (7, 541, 768)
        ...
        x_ln = nn.LayerNorm(epsilon=1e-5, dtype=self.dtype, name='final_ln')(x) # (7, 541, 768)

        info = {}
        if self.add_cls_token:
            cls_vec = x_ln[..., 0, :] # (7, 768)
            info['cls'] = nn.Dense(features=self.hidden_size if self.cls_output_size is None else self.cls_output_size,
                                   dtype=self.dtype, kernel_init=kernel_init, name='cls_proj')(cls_vec) # (7, 768)
            info['seq'] = x_ln[..., 1:, :] # (7, 540, 768)
        else:
            info['seq'] = x_ln
        return info
...
class VisionTransformer(nn.Module):
    ...
    def __call__(self, x, mask = None):
        ...
        *batch_dims, hw, pp3 = x.shape # (7, 540, 768)
        assert hw == (self.output_grid_h * self.output_grid_w) # (18*30)
        assert pp3 == (self.patch_size ** 2) * 3

        # i think no need to normalize here because pixel distributions are roughly the same
        x = nn.Dense(features=self.hidden_size, dtype=self.dtype, kernel_init=kernel_init, name='embedding')(x) # (7, 540, 768)

        if self.do_rotary:
            coords = get_rotary_coordinates_2d(self.output_grid_h, self.output_grid_w, dtype=self.dtype) # (540, 2)
        else:
            coords = None
        t_out = TransformerEncoder(hidden_size=self.hidden_size, dtype=self.dtype,
                                   ...
                                   size_per_head=self.size_per_head)(x, rotary_coords=coords) # 'cls': (7, 768), 'seq': (7, 540, 768) 

        # Attention pool
        assert self.output_grid_h % self.pooling_ratio == 0
        assert self.output_grid_w % self.pooling_ratio == 0
        h2 = self.output_grid_h // self.pooling_ratio # 9
        w2 = self.output_grid_w // self.pooling_ratio # 15
        b2 = int(np.prod(list(batch_dims) + [h2])) # 63

        seq = jnp.reshape(t_out['seq'], [b2, self.pooling_ratio, w2, self.pooling_ratio, self.hidden_size]) # (63,2,15,2,768)
        seq = seq.swapaxes(-4, -3) # (63,15,2,2,768)
        seq = seq.reshape([b2 * w2, self.pooling_ratio ** 2, self.hidden_size]) # (945,4,768)
        
        target_seq = deepcopy(seq)
        rkey = jax.random.PRNGKey(758493)
        seq_mask = jax.random.randint(rkey, [b2 * w2], minval=0, maxval=4)[..., None]
        for idx in range(seq.shape[0]):
            seq = seq.at[idx,seq_mask[idx]].set(jnp.zeros([768], dtype=seq.dtype))

        inputs_q = seq.mean(-2, keepdims=True) # (945,1,768)
        target_q = target_seq.mean(-2, keepdims=True)
        
        multiheadattn = nn.MultiHeadDotProductAttention(num_heads=self.hidden_size // self.size_per_head, dtype=self.dtype, deterministic=True, 
                                                       name='seq_attnpool')
        seq_attnpool = multiheadattn(inputs_q=inputs_q, inputs_kv=seq) # (945,1,768)
        seq_attnpool = seq_attnpool.reshape(list(batch_dims) + [h2 * w2, self.hidden_size]) # (7,135,768)

        t_out['seq_attnpool'] = seq_attnpool # (7, 135, 768)
        
        target_seq_attnpool = multiheadattn(inputs_q=target_q, inputs_kv=target_seq) # (945,1,768)
        target_seq_attnpool = target_seq_attnpool.reshape(list(batch_dims) + [h2 * w2, self.hidden_size]) # (7,135,768)

        t_out['target_seq_attnpool'] = target_seq_attnpool # (7, 135, 768)
...
def one_hot_pool(do_pool, idx, v, num_segments, real_bsize=None):
    ...
    return {'x': attended, 'idx_oh': pointer_oh} # 'x': (1,945,768), 'idx_oh': (1,945,945)
...
class MerlotReserve(nn.Module):
    ...
    def prepare_multimodal_inputs(self, tokens, token_segment_idx=None, token_embs=None, vision_input=None,
                                  audio_spans=None, audio_pointers=None, padding_len=None, video_src_idx=None, mask_patches=None):
        """
        ...
        if (audio_spans is None) or (audio_pointers is None):
            ...
            audio_subpos = jnp.maximum(jnp.cumsum(is_audio_src.astype(jnp.int32), -1) - 1, 0) % self.audio_token_length # (1, 256)

            audio_embs = audio_spans[jnp.arange(B, dtype=jnp.int32)[:, None], audio_ptr, audio_subpos] # (1,256,768)
            token_embs = lax.select(jnp.repeat(is_audio_src[..., None], self.hidden_size, axis=-1), on_true=audio_embs, on_false=token_embs) # (1,256,768)

        token_idx = jnp.tile(1.0 + jnp.arange(L, dtype=self.dtype)[None], [B, 1]) # (B, L); value -> (1 to 256)
        coords = multimodal_rotary_coords(
            ...
            token_idx=token_idx, dtype=self.dtype) # (B, L, 4) -> (1, 256, 4)

        if vision_input is not None:

            hpool = self.output_grid_h // self.vit_pooling_ratio # 18 // 2 -> 9
            wpool = self.output_grid_w // self.vit_pooling_ratio # 30 // 2 -> 15
            img_coords_pool = get_rotary_coordinates_2d(hpool, wpool, dtype=self.dtype) # (9*15, 2) -> (135, 2)

            b_, vis_seq_len, h_ = vision_input.shape # (1, 1080, 768)
            num_pool_segments = vis_seq_len // (hpool * wpool) # 8
            img_coords = jnp.tile(img_coords_pool, [num_pool_segments, 1]) # (1080, 2)
            vis_segment_idx = jnp.arange(num_pool_segments, dtype=jnp.int32).repeat(hpool * wpool) # (1080,)
            print(f"Vision input {hpool}x{wpool}: {vis_seq_len}. num_pool_segments: {num_pool_segments}",
                  flush=True) # Vision input 9x15: 1080. num_pool_segments: 8
            img_coords = jnp.tile(img_coords[None], [B, 1, 1]) # (B, num_pool_segments * hpool * wpool, 2) -> (1, 1080, 2)
            vis_segment_idx = jnp.tile(vis_segment_idx[None], [B, 1]) # (B, num_pool_segments * hpool * wpool) -> (1, 1080)
            img_mm_coords = multimodal_rotary_coords(segment_idx=vis_segment_idx.astype(self.dtype),
                                                   h=img_coords[..., 0], w=img_coords[..., 1], dtype=self.dtype) # (B, num_pool_segments * hpool * wpool, 4) -> (1, 1080, 4)
            assert img_mm_coords.shape[-2] == vis_seq_len 
            coords = jnp.concatenate([coords, img_mm_coords], 1) # (B, num_pool_segments * hpool * wpool + L, 4) -> (1, 1336, 4)
            
            token_embs = jnp.concatenate([token_embs, vision_input], 1) # (B, num_pool_segments * hpool * wpool + L, 768) -> (1, 1336, 768)
        else:
            vis_seq_len = 0
            vis_segment_idx = None

        # Attention mask
        is_valid = (tokens != PADDING) # (B, L)
        if vis_seq_len > 0:
            is_valid = jnp.concatenate([is_valid, jnp.ones([B, vis_seq_len], dtype=is_valid.dtype)], 1) # (B, L+vis_seq_len); vis_seq_len = num_pool_segments * hpool * wpool
        ...
        attn_mask = is_valid[:, None] & is_valid[:, :, None] # (B, L+vis_seq_len, L+vis_seq_len) -> (1, 1336, 1336)
        ...
        return {'x': token_embs, 'rotary_coords': coords, 'attention_mask': attn_mask} # (1, 1336, 768), (1, 1336, 4), (1, 1336, 1336)
