...
from mreserve.lowercase_encoder import get_encoder, START, END, PADDING, MASK, AUDIOSPAN, LTOVPOOL, MASKAUDIO, MASKVIDEO
import math
...
def load_and_resize_img_with_mask(encoded_jpg, config):
    """
    Encoded JPG -> image patches
    :param encoded_jpg: string
    :return: [(H // P) * (W // P), P * P * 3] image
...
def mask_tokens(tokens_ragged, mask_idx, do_audio_span=None, audio_token_length=6, text_span_start_counter=0,
    ...
    mask_idx_onehot = _one_hot(mask_idx, N) # mask_idx.shape
...
def dataset_parser(record, config):
    ...
    * audio: [num_segments, num_audio_spans, T, num_mels]
    ...
    num_audio_spans = num_segments * config['num_audio_subsegments'] # 16*3 = 48
    num_audio_spans_trg = int(num_audio_spans * config['mask_rate']) # 48*0.25 = 12
    num_text2audio_seqs = config['num_text2audio_seqs'] # 1
    num_audio2text_seqs = config['num_audio2text_seqs'] # 1
...
if __name__ == '__main__':
    ...
    dataset = tf.data.TFRecordDataset(['/mnt/disks/persist/sakter/preprocessed/out/train245of256.tfrecord'])
    # # For eager debugging
