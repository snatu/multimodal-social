...
class MoseiDatasetUnaligned(Data.Dataset):
    ...
    def __init__(self, root, clas="train"):
        self.root = root
        self.clas = clas
        if len(MoseiDatasetUnaligned.trainset.y) != 0 and clas != "train":
            print("Data has been previously loaded, fetching from previous lists.")
        else:
            self.load_data()

        if self.clas == "train":
            self.dataset = MoseiDatasetUnaligned.trainset
        elif self.clas == "test":
            self.dataset = MoseiDatasetUnaligned.testset
        elif self.clas == "valid":
            self.dataset = MoseiDatasetUnaligned.validset
    ...
    def load_data(self):
        ...
        dataset = pickle.load(open(gc['data_path'] + 'mosei_senti_data_noalign.pkl', 'rb'))
        ...
        for ds, split_type in [(MoseiDatasetUnaligned.trainset, 'train'), (MoseiDatasetUnaligned.validset, 'valid'),
                               (MoseiDatasetUnaligned.testset, 'test')]:
...
if __name__ == "__main__":
    dataset = MoseiDatasetUnaligned(gc['data_path'])
