# rewrote this to return attention weights
...
def group(xs: List[Tensor], aggr: str) -> Optional[Tensor]:
    if len(xs) == 0:
        ...
        if isinstance(xs[0], tuple): # to be able to pass back attention values from GATConv
            new_xs = [elt[0] for elt in xs]
            other_out = [elt[1] for elt in xs]
            
            out = torch.stack(new_xs, dim=0)
