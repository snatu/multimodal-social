...
class MerlotReserveTVQA(MerlotReserve):
    ...
    def __call__(self, batch):
        ...
        return logits_from_audio, logits_from_text, num, xyz
...
def pred_step(state: train_state.TrainState, batch):
    logits_from_audio, logits_from_text, num, rand = state.apply_fn({'params': state.params}, batch)
...
def val_epoch(state: train_state.TrainState):
    ...
    text_preds.to_csv('robustness/{}_{}_{}_text_preds.csv'.format(args.modality, args.mode, args.percent))
    audio_preds.to_csv('robustness/{}_{}_{}_audio_preds.csv'.format(args.modality, args.mode, args.percent))
    joint_preds.to_csv('robustness/{}_{}_{}_joint_preds.csv'.format(args.modality, args.mode, args.percent))
