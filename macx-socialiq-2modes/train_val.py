...
class LSTM_QAVT_MAC(nn.Module):
    ...
    def forward(self, q, answers, subs, vis):
        if MODE == 'train':
            logits = self.mac(subs, vis, q, answers)
            accumulate(self.mac_running, self.mac)
        else:
            logits = self.mac_running(subs, vis, q, answers)
...
if __name__ == '__main__':
    ...
    for i in range(3):
        ...
        ma.append(max_acc)
        torch.save(net, '/ocean/projects/dmr120014p/sulagna/binaryqa_VT.pth')
        t_end = time()
    ...
    # # ##########################################################################################
    """
    train_dataset = SocialIQ(data_path, 'train', mods={'ac', 't'}, a4=True)
    val_dataset = SocialIQ(data_path, 'val', mods={'ac', 't'}, a4=True)
    ...
    for i in range(3):
        ...
        for epoch in range(num_epochs):
            ...
            print('Training ............')
            MODE = 'train'
            train_acc, train_loss = train(2, a4=True)
            print(f'\nAcc: {train_acc}, Loss: {train_loss}\n')
            print('Evaluating ............')
            MODE = 'eval'
            val_acc, val_loss = validate(2, a4=True)
            print(f'\nAcc: {val_acc}, Loss: {val_loss}\n')
            if (val_acc > max_acc):
                max_acc = val_acc
            print(f'MaxValAcc: {max_acc}\n')
        ma.append(max_acc)
        torch.save(net, '/ocean/projects/dmr120014p/sulagna/mcqa_AcT.pth')
        t_end = time()
        print(f'time: {t_end - t_start}')
        times.append(t_end - t_start)
    print(ma)
